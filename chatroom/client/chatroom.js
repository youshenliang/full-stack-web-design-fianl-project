...
var chatroom_div = function test() {
  ...
  // 根據get參數抓取目前登入的使用者(方便測試用，之後要改成別的方法)
  LOGIN_USER = window.location.href.split("username=")[1];
  if (window.location.href.indexOf("newuser") != -1) {
    console.log("t");
    $("h1").prepend("<center>Registered Successfully</center><br/>");
  }
  $("#title").text("LOGIN AS: " + LOGIN_USER.toUpperCase());
